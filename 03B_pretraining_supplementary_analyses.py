"""
03B_pretraining_supplementary_analyses.py  (rev.2)
=========================================================================
在 03 (QC/角度提取) 之后、04 (模型训练) 之前运行。不训练模型、不做消融。

修订说明（相对 rev.1）:
  [删除] 配对阶段 5940 -> 3815：该步骤为人工筛选，口径见论文 Table 1，
         脚本不再重算，避免与正文数字打架。
  [保留] QC 阶段 3815 -> 建模帧：脚本可精确复现，回应 R1-3 的
         "clearer breakdown of exclusions"。
  [修正] episode 内一致性：只统计 111 个合格 episode（与 04 训练同口径），
         rev.1 的 n=143 混入了弱 episode，不可用。
  [新增] S5c_episode_coverage_by_bcs.csv：帧级通过率 vs episode 级覆盖率并排，
         这是回应 R1-3"选择偏差"质疑的核心证据。
  [降级] S2 只描述覆盖率，不能证明阈值 3 最优；
         性能证据见 04B_threshold_performance.py。

对应审稿人意见：
  R1-3  QC 排除明细 + 帧级/episode 级覆盖率对比
  R2-3  episode 内一致性（仅合格 episode）
  R2    52 个 animal-level groups 的构成
...
输入（相对路径，CWD = 项目根目录）：
  ...
  ./03_quality_controlled_dorsal_angle/03_bad_reason_counts.csv

输出目录：./03B_pretraining_supplementary_analyses/
  S1_qc_exclusion_breakdown.csv       QC 阶段排除明细（投稿用）
  S1b_exclusion_reason_detail.csv     排除原因细项（降序 + 占比）
  S2_threshold_coverage.csv           阈值-覆盖率（描述性）
  S3_intra_episode_consistency.csv    episode 内一致性汇总（仅 111 合格）
  S3a_intra_episode_per_episode.csv   逐 episode 明细
  S4_animal_group_composition.csv     52 animal-level groups 构成
  S4a_animal_group_summary.csv
  S5a_frame_qc_pass_rate_by_bcs.csv   帧级通过率（按 BCS）
  S5b_episode_coverage_by_fold.csv    episode 覆盖率（按 fold）
  S5c_episode_coverage_by_bcs.csv     episode 覆盖率（按 BCS）<< 关键
=========================================================================
"""
...
def compute_eligible_ids(bag_df: pd.DataFrame, min_frames: int = None) -> set:
    """
    合格 episode: good_angle_frame_count >= min_frames 且 median angle 有效。
    与 04 训练脚本筛选逻辑一致。
    """
