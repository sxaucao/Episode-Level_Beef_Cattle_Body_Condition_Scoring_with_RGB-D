#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""最终模型的辅助监督补充实验（单文件，可直接运行）。

把本文件放到 cattle 数据根目录，与03质量控制目录、04结果目录并列。
默认复用你已完成的 run_20260907_055223_144834，只新增无辅助监督的三折训练。
  python 05_auxiliary_head_ablation.py
  python 05_auxiliary_head_ablation.py --check-only
  python 05_auxiliary_head_ablation.py --groups no_aux low_only high_only
  python 05_auxiliary_head_ablation.py --baseline-run "另一基线结果目录"

训练模型、预处理、采样、损失实现和快照选择沿用已经核验的04脚本。
每组从相同种子的随机初始化重新训练，只改变辅助损失权重；不载入训练后的权重继续训练。
默认70轮、160x160、12帧、batch8等参数从基线effective_config.json读取。
若基线采用其他设置，沿用其实际设置，避免对照实验混入额外参数变化。
输出：三折均值/标准差、配对差值、逐Episode对照、类别召回率和相反方向错误计数。
依赖与04脚本相同；Windows多进程入口已保留。
"""
...
# === AUXILIARY SUPERVISION ABLATION ADDITION ===

BASELINE_ENGINE_SHA256 = "0672897b0c36f47fddc327aad462c9c6bc4560d0fb205a1b8786a2099cee1fa1"
DEFAULT_BASELINE_RUN = "04_paper_aligned_comil_results/run_20260907_055223_144834"
AUX_GROUPS = {
    "low_high": (0.45, 0.45),
    "no_aux": (0.0, 0.0),
    "low_only": (0.45, 0.0),
...
COMPARISON_METRICS = [
    "acc5", "macro_f1_5", "weighted_f1_5", "qwk5", "mae_bcs", "within_one_acc",
    "acc3_final", "macro_f1_3_final", "weighted_f1_3_final", "qwk3_final",
    "lean_recall_final", "high_recall_final", "lean_precision_final", "high_precision_final",
    "lean_to_high_errors", "high_to_lean_errors", "bcs2_to_bcs6_errors",
    "bcs6_to_bcs2_or_3_errors", "test_coverage",
]
SPLIT_COLUMNS = ["test_fold", "split", "bag_id", "animal_id", "bcs_raw", "eligible_for_prediction"]


def supplement_csv(path, required=()):
    path = Path(path)
    if not path.is_file():
        raise FileNotFoundError(f"缺少 {path}。--baseline-run 应指向包含final子目录的完整原始run目录。")
    df = pd.read_csv(path, encoding="utf-8-sig", dtype={"bag_id": str, "animal_id": str})
...
def comparison_mean_std(values):
    values = np.asarray(values, dtype=float)
    values = values[np.isfinite(values)]
    return (float(values.mean()) if len(values) else float("nan"),
            float(values.std(ddof=1)) if len(values) > 1 else float("nan"), len(values))


def validate_result_set(fold_df, pred_df, bags, cfg, label):
    """既有基线与新组均需有相同三折、相同测试Episode；由预测重新核算指标。"""
    expected = {b["bag_id"]: b for b in bags if b["eligible"]}
    folds = sorted({b["fold_id"] for b in bags})
    if sorted(fold_df["fold"].tolist()) != folds or fold_df["fold"].duplicated().any():
        raise ValueError(f"{label}: 必须包含完整三折且每折恰好一行")
    if pred_df.bag_id.duplicated().any() or set(pred_df.bag_id) != set(expected):
        raise ValueError(f"{label}: OOF预测的Episode集合不一致或存在重复")
    for row in pred_df.to_dict("records"):
        actual = expected[row["bag_id"]]
        for column, expected_value in (("fold", actual["fold_id"]), ("true_label5", actual["label5"]),
                                       ("true_label3", actual["label3"]), ("animal_id", actual["animal_id"])):
            if row[column] != expected_value:
                raise ValueError(f"{label}/{row['bag_id']}: {column}与基线数据不一致")
        if not 0 <= row["pred_label5"] <= 4 or int(row["pred_label5"]) != row["pred_label5"]:
            raise ValueError(f"{label}: 预测标签无效")
        if row["pred_label3_final"] != label5_to_label3(row["pred_label5"]):
            raise ValueError(f"{label}: 三级类别必须由五级预测映射")
    prob_columns = [f"prob_bcs{c}" for c in range(2, 7)]
    if not set(prob_columns).issubset(pred_df.columns):
        raise ValueError(f"{label}: 缺少五级预测概率")
    probabilities = pred_df[prob_columns].to_numpy(dtype=float)
    if (not np.isfinite(probabilities).all() or (probabilities < 0).any() or (probabilities > 1).any()
            or not np.allclose(probabilities.sum(axis=1), 1.0, atol=1e-6)
            or not np.array_equal(probabilities.argmax(axis=1), pred_df.pred_label5.to_numpy())):
        raise ValueError(f"{label}: 概率或argmax预测不一致")
    for fold in folds:
        subset = pred_df[pred_df.fold == fold]
        recorded = fold_df[fold_df.fold == fold].iloc[0]
        metrics = calculate_metrics(subset.true_label5, subset.pred_label5, cfg)
        raw_count = sum(b["fold_id"] == fold for b in bags)
        metrics.update(raw_test_bags=raw_count, eligible_test_bags=len(subset), test_coverage=len(subset) / raw_count)
        for key, value in metrics.items():
            if key not in recorded or not np.isclose(value, recorded[key], atol=1e-8, rtol=1e-7, equal_nan=True):
                raise ValueError(f"{label}/fold{fold}: {key}与逐Episode预测计算结果不一致")


def audit_existing_baseline(base_path, baseline_dir, output_name):
    require_torch()
    recorded_config = supplement_json(baseline_dir / "effective_config.json")
    baseline_cfg = Config(**recorded_config).validate()
    if not (baseline_cfg.lambda_low == 0.45 and baseline_cfg.lambda_high == 0.45):
        raise ValueError("复用基线的lambda_low与lambda_high应当均为0.45")
    provenance = supplement_json(baseline_dir / "run_provenance.json")
    if provenance.get("source_sha256", {}).get("code") != BASELINE_ENGINE_SHA256:
        raise ValueError("基线不是本补充脚本所对应的已核验04版本；请使用原run_20260907_055223_144834结果。")
    runtime = {"torch": str(torch.__version__), "numpy": np.__version__, "pandas": pd.__version__}
    mismatches = {key: {"baseline": provenance.get(key), "current": value}
                  for key, value in runtime.items() if provenance.get(key) != value}
    if mismatches:
        raise ValueError(f"复用旧结果需要保持原PyTorch/numpy/pandas环境，检测到差异: {mismatches}。请使用完成基线实验的Python解释器。")
    cfg = replace(baseline_cfg, base_path=str(base_path), out_dir=output_name)
    for key in ("frame_qc_csv", "bag_qc_csv"):
        path = resolve_path(base_path, getattr(cfg, key))
        if not path.is_file():
            raise FileNotFoundError(f"找不到数据文件: {path}")
        digest = hashlib.sha256(path.read_bytes()).hexdigest()
        if digest != provenance["source_sha256"].get(key):
            raise ValueError(f"{key}内容与基线运行时不同，不能作为仅改变辅助监督的对照")
    bags = build_bags(cfg)
    splits = make_splits(bags, cfg)
    generated_rows = []
    for fold, parts in splits.items():
        for role, values in parts.items():
            for b in values:
                generated_rows.append({"test_fold": fold, "split": role, "bag_id": b["bag_id"],
                                       "animal_id": b["animal_id"], "bcs_raw": b["bcs_raw"],
                                       "eligible_for_prediction": int(b["eligible"])})
    recorded_split = supplement_csv(baseline_dir / "split_manifest.csv", SPLIT_COLUMNS)
    sort_columns = ["test_fold", "split", "bag_id"]
    left = recorded_split[SPLIT_COLUMNS].sort_values(sort_columns).reset_index(drop=True)
    right = pd.DataFrame(generated_rows)[SPLIT_COLUMNS].sort_values(sort_columns).reset_index(drop=True)
    if left.to_dict("records") != right.to_dict("records"):
        raise ValueError("当前生成的动物划分或门控资格与已完成的基线不同")
    folder = baseline_dir / "final"
    fold_df = supplement_csv(folder / "fold_metrics.csv", ["fold", "best_epoch"] + COMPARISON_METRICS)
    pred_df = supplement_csv(folder / "oof_predictions.csv", ["bag_id", "animal_id", "fold", "true_label5", "true_label3", "pred_label5", "pred_label3_final"])
    validate_result_set(fold_df, pred_df, bags, cfg, "low_high baseline")
    device = choose_device(cfg)
    audits = []
    for fold, parts in splits.items():
        checkpoint_path = folder / f"fold{fold}_best.pth"
        if not checkpoint_path.is_file():
            raise FileNotFoundError(f"缺少基线检查点: {checkpoint_path}")
        checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
        if checkpoint.get("variant") != "final" or json_safe(checkpoint.get("config")) != json_safe(recorded_config):
            raise ValueError(f"fold{fold}: 基线检查点的模型或配置与记录不一致")
        expected_parts = {role: select_variant_bags(values, VARIANTS["final"]) for role, values in parts.items()}
        if any(not values for values in expected_parts.values()):
            raise ValueError(f"fold{fold}存在空训练/验证/测试集合")
        for role, values in expected_parts.items():
            if checkpoint["split_bag_ids"][role] != [b["bag_id"] for b in values]:
                raise ValueError(f"fold{fold}/{role}: Episode划分或顺序与基线不一致")
            if set(checkpoint["split_animal_ids_raw"][role]) != {b["animal_id"] for b in parts[role]}:
                raise ValueError(f"fold{fold}/{role}: 动物划分与基线不一致")
        mean, std = fit_angle_scaler(expected_parts["train"])
        counts, weights = inverse_class_weights(expected_parts["train"])
        for key, value in (("angle_mean", mean), ("angle_std", std), ("class_counts", counts), ("class_weights", weights)):
            if not np.allclose(value, checkpoint[key], atol=1e-6, rtol=1e-6):
                raise ValueError(f"fold{fold}: {key}与基线不一致")
        original_metrics = fold_df[fold_df.fold == fold].iloc[0]
        if (checkpoint["best_epoch"] != original_metrics["best_epoch"]
                or not np.isclose(checkpoint["best_val_score"], original_metrics["best_val_score"])):
            raise ValueError(f"fold{fold}: 最佳快照与指标文件不一致")
        parameters = supplement_json(folder / f"fold{fold}_training_parameters.json")
        if parameters["amp_effective"] != bool(cfg.amp and device.type == "cuda"):
            raise ValueError("当前实际AMP/设备设置与基线不同，请使用原运行环境")
        state = checkpoint["model_state"]
        if tuple(state["bcs_head.weight"].shape) != (5, cfg.shared_dim):
            raise ValueError("基线不是五级Softmax主头")
        if tuple(state["shared_mapping.weight"].shape) != (cfg.shared_dim, cfg.frame_fusion_dim + cfg.angle_embedding_dim):
            raise ValueError("基线未采用相同的视觉与角度融合结构")
        history = supplement_csv(folder / f"fold{fold}_training_history.csv", ["epoch", "val_score", "val_macro_f1_3", "val_qwk5", "val_extreme_penalty"])
        if history.epoch.tolist() != list(range(1, cfg.epochs + 1)):
            raise ValueError(f"fold{fold}: 基线没有完整训练规定的epoch")
        score = (history.val_macro_f1_3 + cfg.selection_qwk_weight * history.val_qwk5.fillna(-1.0)
                 - cfg.selection_extreme_weight * history.val_extreme_penalty)
        if not np.allclose(score, history.val_score):
            raise ValueError(f"fold{fold}: 基线快照选择公式不同")
        best_history = history.loc[history.val_score.idxmax()]
        if (int(best_history.epoch) != checkpoint["best_epoch"]
                or not np.isclose(best_history.val_score, checkpoint["best_val_score"])):
            raise ValueError(f"fold{fold}: 记录的最佳快照不是验证得分最高的最早epoch")
        audits.append({"fold": fold, "same_split_and_order": True, "same_scaler_and_weights": True,
                       "baseline_best_epoch": int(checkpoint["best_epoch"]),
                       "val_bcs_counts": dict(Counter(b["bcs_raw"] for b in expected_parts["val"]))})
        del checkpoint
    check_image_paths(bags, [VARIANTS["final"]])
    audit = {"baseline_run": str(baseline_dir), "engine_sha256": BASELINE_ENGINE_SHA256,
             "baseline_files_sha256": {str(path.relative_to(baseline_dir)): hashlib.sha256(path.read_bytes()).hexdigest()
                                       for path in [baseline_dir / "effective_config.json", baseline_dir / "run_provenance.json",
                                                    baseline_dir / "split_manifest.csv", folder / "fold_metrics.csv", folder / "oof_predictions.csv"]},
             "runtime_checked": runtime, "source_csv_sha256": provenance["source_sha256"],
             "raw_episodes": len(bags), "eligible_episodes": sum(b["eligible"] for b in bags),
             "fold_audit": audits,
             "image_content_note": "历史基线未保存逐图像内容摘要；请保持RGB/npy原始文件不变。"}
    return cfg, bags, splits, device, fold_df, pred_df, audit


def initialization_digest(cfg, fold):
    """重建相同种子的初始网络；实际训练仍由原train_one_fold重新设置种子。"""
    set_seed(cfg.seed + fold, cfg.deterministic)
    model = AngleAwareRGBDepthLowHighCOMIL(cfg, VARIANTS["final"])
...
def write_auxiliary_comparison(results, cfg, bags, output_dir):
    """仅作配对描述性比较；不把同一动物的多个Episode视为独立统计检验样本。"""
    summaries, all_folds, differences, pairs, outcomes, recalls, extreme_rows = [], [], [], [], [], [], []
    reference_folds, reference_pred = results["low_high"]
    reference_folds = reference_folds.set_index("fold").sort_index()
    reference_pred = reference_pred.set_index("bag_id").sort_index()
    for name, (fold_df, pred_df) in results.items():
        validate_result_set(fold_df, pred_df, bags, cfg, name)
        current = fold_df.set_index("fold").sort_index()
        for row in fold_df.to_dict("records"):
            all_folds.append({"group": name, **row})
        for metric in COMPARISON_METRICS:
            mean, std, n = comparison_mean_std(current[metric])
            direction = "lower" if metric == "mae_bcs" or metric.endswith("_errors") else "higher"
            if metric == "test_coverage":
                direction = "must_match"
            summaries.append({"group": name, "lambda_low": AUX_GROUPS[name][0], "lambda_high": AUX_GROUPS[name][1],
                              "metric": metric, "mean": mean, "std": std, "n_valid_folds": n, "better_direction": direction})
            if name != "low_high":
                for fold in current.index:
                    baseline = reference_folds.loc[fold, metric]
                    trial = current.loc[fold, metric]
                    differences.append({"group": name, "fold": fold, "metric": metric,
                                        "low_high": baseline, "group_value": trial,
                                        "group_minus_low_high": trial - baseline, "better_direction": direction})
        indexed = pred_df.set_index("bag_id").loc[reference_pred.index]
        for fold in [*current.index.tolist(), "pooled"]:
            subset = pred_df if fold == "pooled" else pred_df[pred_df.fold == fold]
            for label in range(5):
                support = int((subset.true_label5 == label).sum())
                correct = int(((subset.true_label5 == label) & (subset.pred_label5 == label)).sum())
                recalls.append({"group": name, "fold": fold, "bcs": label + 2, "support": support,
                                "correct": correct, "recall": correct / support if support else float("nan")})
        pooled_metrics = calculate_metrics(pred_df.true_label5, pred_df.pred_label5, cfg)
        for key in [k for k in COMPARISON_METRICS if k.endswith("_errors")]:
            extreme_rows.append({"group": name, "error_type": key, "oof_total": pooled_metrics[key]})
        if name == "low_high":
            continue
        actual = indexed.true_label5.to_numpy()
        base_pred, trial_pred = reference_pred.pred_label5.to_numpy(), indexed.pred_label5.to_numpy()
        for bid, yt, old, new in zip(indexed.index, actual, base_pred, trial_pred):
            pairs.append({"group": name, "bag_id": bid, "animal_id": indexed.loc[bid, "animal_id"],
                          "fold": int(indexed.loc[bid, "fold"]), "true_bcs": int(yt + 2),
                          "low_high_pred_bcs": int(old + 2), "group_pred_bcs": int(new + 2),
                          "low_high_abs_error": int(abs(old - yt)), "group_abs_error": int(abs(new - yt)),
                          "low_high_correct": int(old == yt), "group_correct": int(new == yt)})
        tests = {
            "Acc5": (base_pred == actual, trial_pred == actual),
            "Acc3": (LABEL5_TO_LABEL3[base_pred] == LABEL5_TO_LABEL3[actual], LABEL5_TO_LABEL3[trial_pred] == LABEL5_TO_LABEL3[actual]),
            "Within_one": (np.abs(base_pred - actual) <= 1, np.abs(trial_pred - actual) <= 1),
        }
        for metric, (left, right) in tests.items():
            outcomes.append({"group": name, "metric": metric, "both_correct": int((left & right).sum()),
                             "low_high_only_correct": int((left & ~right).sum()),
                             "group_only_correct": int((~left & right).sum()),
                             "both_wrong": int((~left & ~right).sum()), "n_paired_episodes": len(actual)})
    delta_summary = []
    difference_groups = pd.DataFrame(differences).groupby(["group", "metric"]) if differences else []
    for (group, metric), values in difference_groups:
        mean, std, n = comparison_mean_std(values.group_minus_low_high)
        delta_summary.append({"group": group, "metric": metric, "mean_group_minus_low_high": mean,
                              "std_of_paired_differences": std, "n_paired_folds": n,
                              "better_direction": values.better_direction.iloc[0]})
    write_csv(output_dir / "auxiliary_ablation_summary.csv", summaries)
    write_csv(output_dir / "auxiliary_ablation_fold_metrics.csv", all_folds)
    write_csv(output_dir / "auxiliary_ablation_paired_deltas.csv", differences)
    write_csv(output_dir / "auxiliary_ablation_delta_mean_std.csv", delta_summary)
    write_csv(output_dir / "paired_oof_predictions.csv", pairs)
    write_csv(output_dir / "paired_outcome_counts.csv", outcomes)
    write_csv(output_dir / "class_recall_comparison.csv", recalls)
    write_csv(output_dir / "extreme_error_totals.csv", extreme_rows)
    print("\n三折平均结果（同一批测试Episode）", flush=True)
    for metric in ("acc5", "macro_f1_5", "qwk5", "mae_bcs", "within_one_acc", "macro_f1_3_final", "lean_recall_final", "high_recall_final"):
        cells = [f"{r['group']}={r['mean']:.4f}±{r['std']:.4f}" for r in summaries if r["metric"] == metric]
        print(metric + ": " + " | ".join(cells), flush=True)
    return summaries


def auxiliary_main(argv=None):
    parser = argparse.ArgumentParser(description="最终模型辅助监督消融：复用既有Low+High基线，默认补跑无辅助监督组。")
    parser.add_argument("--base-path", type=Path, default=Path(__file__).resolve().parent, help="数据根目录；默认本脚本所在目录")
    parser.add_argument("--baseline-run", type=str, default=DEFAULT_BASELINE_RUN, help="已有run目录，须含final子目录")
    parser.add_argument("--out-dir", default="05_auxiliary_head_ablation_results")
    parser.add_argument("--groups", nargs="+", choices=["no_aux", "low_only", "high_only", "all"], default=["no_aux"])
    parser.add_argument("--check-only", action="store_true", help="核对基线与输入文件，输出检查记录，不训练")
    parser.add_argument("--self-test", action="store_true", help="运行原训练引擎的合成逻辑与模型前后向测试")
    args = parser.parse_args(argv)
    if args.self_test:
        self_test()
        return
    base = args.base_path.expanduser().resolve()
    baseline_dir = resolve_path(base, args.baseline_run).resolve()
    groups = ["no_aux", "low_only", "high_only"] if "all" in args.groups else list(dict.fromkeys(args.groups))
    print(f"复用基线：{baseline_dir}", flush=True)
    cfg, bags, splits, device, baseline_folds, baseline_predictions, audit = audit_existing_baseline(base, baseline_dir, args.out_dir)
    output_dir = resolve_path(base, args.out_dir) / ("run_" + datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S_%f"))
    output_dir.mkdir(parents=True, exist_ok=False)
    write_json(output_dir / "baseline_reuse_audit.json", audit)
    save_run_metadata(cfg, output_dir)
    save_data_audit(bags, splits, cfg, output_dir)
    reference_dir = output_dir / "low_high_reference"
    reference_dir.mkdir()
    write_csv(reference_dir / "fold_metrics.csv", baseline_folds.to_dict("records"))
    write_csv(reference_dir / "oof_predictions.csv", baseline_predictions.to_dict("records"))
    plan = {"reference_group": "low_high", "reference_run": str(baseline_dir),
            "planned_groups": groups, "completed_groups": [], "check_only": args.check_only,
            "epochs_per_fold": cfg.epochs, "seed": cfg.seed,
            "loss_weights": {name: AUX_GROUPS[name] for name in ["low_high"] + groups},
            "changed_training_parameters": ["lambda_low", "lambda_high"],
            "parameterized_modules_kept_identical": True,
            "training_start": "same seeded random initialization, no pretrained baseline weights loaded",
            "selection_rule": "same validation composite score as the manuscript, separately for each group",
            "statistical_scope": "one seed, fixed animal-wise 3 folds, descriptive paired comparisons; no significance claim",
            "training_engine_sha256": BASELINE_ENGINE_SHA256}
    write_json(output_dir / "experiment_plan.json", plan)
    if args.check_only:
        print(f"基线核对通过，未启动训练。检查记录：{output_dir}", flush=True)
        return
    initial_hashes = {fold: initialization_digest(cfg, fold) for fold in splits}
    write_json(output_dir / "initialization_audit.json", {
        "reconstructed_reference_initialization": initial_hashes,
        "note": "旧基线未保存初始权重；按相同引擎、环境、配置和种子重建，用于核对所有新组的初始参数。"})
    results = {"low_high": (baseline_folds, baseline_predictions)}
    for group in groups:
        low, high = AUX_GROUPS[group]
        current = replace(cfg, lambda_low=low, lambda_high=high)
        hashes = {fold: initialization_digest(current, fold) for fold in splits}
        if hashes != initial_hashes:
            raise RuntimeError("改变辅助损失权重意外改变了模型初始化")
        group_dir = output_dir / group
        group_dir.mkdir()
        save_run_metadata(current, group_dir)
        write_json(group_dir / "condition.json", {"group": group, "lambda_low": low, "lambda_high": high,
                                                  "initial_state_sha256": hashes})
        print(f"\n补充组 {group}: lambda_low={low}, lambda_high={high}；{len(splits)}折×{current.epochs}轮。", flush=True)
        # 保留完全相同的网络结构、初始化与随机数消耗，仅通过损失系数关闭监督。
        # 例如no_aux的两个辅助头仍被构造，其梯度贡献为0；主损失与推理结构不变。
        fold_rows, predictions = run_variant(bags, splits, current, VARIANTS["final"], device, group_dir)
        results[group] = (pd.DataFrame(fold_rows), pd.DataFrame(predictions))
        write_auxiliary_comparison(results, cfg, bags, output_dir)
        plan["completed_groups"].append(group)
        write_json(output_dir / "experiment_plan.json", plan)
    print(f"\n补充实验完成：{output_dir}", flush=True)
    print("请查看 auxiliary_ablation_summary.csv、auxiliary_ablation_delta_mean_std.csv 和 paired_oof_predictions.csv。", flush=True)


if __name__ == "__main__":
    try:
        auxiliary_main()
    except (ValueError, FileNotFoundError, RuntimeError, FloatingPointError, KeyError) as exc:
