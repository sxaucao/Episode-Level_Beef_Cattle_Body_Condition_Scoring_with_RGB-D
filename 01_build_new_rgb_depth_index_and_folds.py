...
def greedy_split_animals(animal_rows, bag_rows):
    ...
    for a in animal_rows:
        ...
        # 稀有程度：包含 BCS2 / BCS6 的 animal 优先分
        rarity_score = 0
        if bcs_counter[2] > 0:
            rarity_score += 10
        if bcs_counter[6] > 0:
...
def main():
    ...
    (BASE_PATH / "../01_build_new_rgb_depth_index_and_folds").mkdir(parents=True, exist_ok=True)
    # 输出文件
    frame_csv = BASE_PATH / "../01_build_new_rgb_depth_index_and_folds/01_new_rgb_depth_frame_index.csv"
    bag_csv = BASE_PATH / "../01_build_new_rgb_depth_index_and_folds/01_new_rgb_depth_bag_inventory.csv"
    animal_csv = BASE_PATH / "../01_build_new_rgb_depth_index_and_folds/01_new_rgb_depth_animal_inventory.csv"
    fold_csv = BASE_PATH / "../01_build_new_rgb_depth_index_and_folds/02_new_rgb_depth_5fold_cowwise.csv"
    summary_csv = BASE_PATH / "../01_build_new_rgb_depth_index_and_folds/02_new_rgb_depth_fold_summary.csv"
