...
QC_FRAME_CSV = Path("./03_quality_controlled_dorsal_angle/03_qc_frame_dorsal_angle.csv")

QC_BAG_CSV = Path("./03_quality_controlled_dorsal_angle/03_qc_bag_dorsal_angle.csv")

OUT_DIR = Path("./04_full_ablation_results")
...
def set_seed(seed: int):
    # Make robust to numpy integer types from pandas/numpy fold IDs.
    seed = int(seed)
    random.seed(seed)
    np.random.seed(seed)
...
def load_frame_index(quality_gated=False, min_good_frames=3, require_angle=False):
    """
    Returns:
        frame_df: frame-level dataframe
        bag_df: bag-level dataframe with one row per bag
    """
    if quality_gated:
        ...
        # Eligible bags: enough reliable frames and valid angle.
        eligible = qc_bag[
...
def mean_std_table(metrics_list):
    ...
    for k in keys:
        ...
        # Some bookkeeping fields are strings, e.g. variant.
        # They should not be included in mean ± std numeric summaries.
...
def summarize_all_variants(results):
    ...
    summary_path = OUT_DIR / "04_ablation_summary_table.csv"
    extreme_path = OUT_DIR / "04_ablation_extreme_error_table.csv"
...
def plot_summary_bars(summary_df):
    ...
    fig.savefig(OUT_DIR / "04_ablation_main_metrics.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_extreme_bars(extreme_df):
    ...
    ax.legend(fontsize=7, ncol=2)
    fig.savefig(OUT_DIR / "04_ablation_extreme_errors.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
