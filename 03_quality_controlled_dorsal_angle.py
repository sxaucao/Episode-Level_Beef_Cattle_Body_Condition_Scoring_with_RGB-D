...
FRAME_CSV = Path("./02_make_longitudinal_protocol_splits/02A_animalwise_3fold.csv")

OUT_DIR = Path("./03_quality_controlled_dorsal_angle")
...
def rotate_mask_and_depth(mask, closeness, rotate_deg):
    """
    Rotate mask and closeness by rotate_deg.
    """
    if not HAS_SCIPY:
...
def profile_fit_quality(prof):
    ...
    try:
        px = np.asarray(prof["profile_x"], dtype=np.float64)
        py = np.asarray(prof["profile_y_smooth"], dtype=np.float64)
        ridge_x = float(prof["ridge_x"])
        width = max(float(prof["width"]), 1.0)

        for side in ("left", "right"):
            sel = prof.get("%s_sel" % side)
            m = prof.get("m_%s" % side)
            b = prof.get("b_%s" % side)

            if sel is None or m is None or b is None:
                continue
            ...
            if ss_tot > 1e-12:
                res["r2_%s" % side] = 1.0 - ss_res / ss_tot

            res["rmse_%s" % side] = float(np.sqrt(ss_res / n))

    except Exception:
        pass

    vals = [v for v in (res["r2_left"], res["r2_right"]) if not np.isnan(v)]
    if vals:
        res["r2_min"] = float(np.min(vals))
        res["r2_mean"] = float(np.mean(vals))
...
def extract_all_angles():
    ...
    print("=" * 100)
    print("Step 03: Quality-controlled dorsal ridge angle extraction")
    ...
    for bag_idx, (bag_id, brs) in enumerate(sorted(bags.items(), key=lambda x: x[0]), start=1):
        ...
        for r in selected:
            ...
            if debug is not None:
                for _pi, _prof in enumerate(debug.get("profiles", []) or []):
                    try:
                        _fq = profile_fit_quality(_prof)
                    except Exception:
                        continue

                    if not np.isnan(_fq["r2_min"]):
                        frame_r2_min_vals.append(_fq["r2_min"])
                    if not np.isnan(_fq["r2_mean"]):
                        frame_r2_mean_vals.append(_fq["r2_mean"])

                    profile_fitq_rows.append({
                        "bag_id": r["bag_id"],
                        ...
                        "angle": float(_prof["angle"]),
                        "yc": int(_prof["yc"]),
                        "width": float(_prof["width"]),
                        "ridge_rel_pos": float(_prof["ridge_rel_pos"]),
                        "ridge_prominence": float(_prof["ridge_prominence"]),
                        "peak_count": int(_prof["peak_count"]),
                        "r2_left": _fq["r2_left"],
                        "r2_right": _fq["r2_right"],
                        "r2_min": _fq["r2_min"],
                        "r2_mean": _fq["r2_mean"],
                        "rmse_left": _fq["rmse_left"],
                        "rmse_right": _fq["rmse_right"],
                        "n_left": int(_fq["n_left"]),
                        "n_right": int(_fq["n_right"]),
                    })

            if frame_r2_min_vals:
                frame_r2_min_median = float(np.median(frame_r2_min_vals))
                frame_r2_min_min = float(np.min(frame_r2_min_vals))
                frame_r2_mean_median = float(np.median(frame_r2_mean_vals)) if frame_r2_mean_vals else np.nan
                frame_n_low_r2 = int(sum(1 for v in frame_r2_min_vals if v < FIT_R2_DIAG_THRESHOLD))
            else:
                frame_r2_min_median = np.nan
                frame_r2_min_min = np.nan
                frame_r2_mean_median = np.nan
                frame_n_low_r2 = 0
            ...
            frame_rows.append({
                ...
                # R1-8 拟合优度诊断（仅记录）
                "frame_r2_min_median": frame_r2_min_median,
                "frame_r2_min_min": frame_r2_min_min,
                "frame_r2_mean_median": frame_r2_mean_median,
                "frame_n_low_r2": frame_n_low_r2,
    ...
    frame_csv = OUT_DIR / "03_qc_frame_dorsal_angle.csv"
    bag_csv = OUT_DIR / "03_qc_bag_dorsal_angle.csv"
    good_frame_csv = OUT_DIR / "03_good_frame_index.csv"
    bad_reason_csv = OUT_DIR / "03_bad_reason_counts.csv"
    profile_fitq_csv = OUT_DIR / "03_profile_fit_quality.csv"
    ...
    if profile_fitq_rows:
        pd.DataFrame(profile_fitq_rows).to_csv(
            profile_fitq_csv, index=False, encoding="utf-8-sig"
        )
    ...
    if profile_fitq_rows:
        print(profile_fitq_csv)
        print(f"  profile-level fit-quality records: {len(profile_fitq_rows)}")
...
def summarize_and_test(frame_df, bag_df):
    ...
    summary_all_csv = OUT_DIR / "03_all_bag_angle_summary_by_bcs.csv"
    summary_good_csv = OUT_DIR / "03_good_bag_angle_summary_by_bcs.csv"
    summary_frame_good_csv = OUT_DIR / "03_good_frame_angle_summary_by_bcs.csv"
    ...
    quality_counts_csv = OUT_DIR / "03_good_frame_counts_by_bcs.csv"
    phase_counts_csv = OUT_DIR / "03_phase_counts_by_bcs.csv"
    ...
    report_path = OUT_DIR / "03_qc_dorsal_angle_report.txt"
...
def make_plots(valid_bag, good_bag, good_frame):
    if len(valid_bag) > 0:
        ...
        plt.tight_layout()
        plt.savefig(OUT_DIR / "03_all_bag_angle_by_bcs.png", dpi=300)
        plt.close()
    ...
    if len(good_bag) > 0:
        ...
        plt.tight_layout()
        plt.savefig(OUT_DIR / "03_good_bag_angle_by_bcs.png", dpi=300)
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(OUT_DIR / "03_good_bag_angle_by_management.png", dpi=300)
        plt.close()
    ...
    if len(good_frame) > 0:
        ...
        plt.tight_layout()
        plt.savefig(OUT_DIR / "03_good_frame_angle_by_bcs.png", dpi=300)
        plt.close()
...
def make_debug_figures(frame_df):
    ...
    pd.DataFrame(rows).to_csv(
        dbg_dir / "03_debug_sample_index.csv",
        index=False,
        encoding="utf-8-sig",
    )
