...
SRC_11 = Path("./06_tables_and_figures_outputs")
SRC_12 = Path("./04_full_ablation_results")
SRC_13 = Path("./07_final_tables_figures")

FINAL_VARIANT_DIR = SRC_12 / "qg_rgb_depth_lowhigh_with_angle"

OUT_DIR = BASE_PATH / "08_manuscript_package"
...
def copy_file(src, dst_dir, dst_name=None, category="", note="", required=True):
    ...
    Record all successful copies and missing files in manifest.
    """
    src = Path(src)
...
def collect_main_figures():
    ...
    Figure 1 and Figure 4 are usually manually drawn high-quality diagrams.
    This script records them as manual items unless corresponding source files
    are found in the expected locations.
    """

    # Figure 1 workflow: manual item unless user later provides it.
    copy_both_ext(
        SRC_11 / "figures",
    ...
    # Figure 5 final confusion matrices from final variant in 04 results.
    copy_file(
        FINAL_VARIANT_DIR / "aggregate_cm5.png",
        FIG_MAIN_DIR,
        "Figure05A_final_model_5class_confusion_matrix.png",
        category="main_figure",
        note="Final model = qg_rgb_depth_lowhigh_with_angle; copied from 04 ablation result.",
        required=True,
    )
    ...
    copy_file(
        ...
        category="main_figure",
        note="Final ordinal-derived Lean/Ideal/High confusion matrix from 04 ablation result.",
        required=True,
    )
    ...
    # Figure 7 and 8 from 07 final tables/figures.
    copy_both_ext(
        SRC_13 / "figures",
        ...
        category="main_figure",
        note="Updated using 04_full_ablation_results.",
        required=True,
    )
...
def collect_main_tables():
    ...
    # Table 3 final main results from 07
    for ext in [".csv", ".xlsx"]:
        copy_file(
            ...
            category="main_table",
            note="Final main results using 04 ablation source of truth.",
            required=(ext == ".csv"),
        )
    ...
    for ext in [".csv", ".xlsx"]:
        copy_file(
            ...
            category="main_table",
            note="Extreme error comparison updated with 04 results.",
            required=(ext == ".csv"),
        )
...
def collect_supplementary():
    ...
    for name in [
        "04_ablation_summary_table.csv",
        "04_ablation_extreme_error_table.csv",
        "04_ablation_main_metrics.png",
        "04_ablation_extreme_errors.png",
    ]:
    ...
    for name in [
        ...
        copy_file(
            ...
            category="supplementary",
            note="Final model detailed report/predictions from 04 ablation.",
            required=False,
        )
    ...
    copy_file(
        BASE_PATH / "03_quality_controlled_dorsal_angle" / "03_qc_bag_dorsal_angle.csv",
        supp_tables_dir,
    ...
    copy_file(
        BASE_PATH / "03_quality_controlled_dorsal_angle" / "03_good_frame_index.csv",
        supp_tables_dir,
